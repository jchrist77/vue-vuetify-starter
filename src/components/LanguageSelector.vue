...
<template>
  <v-menu>
    ...
    <v-list density="compact" class="py-0" :value="appStore.language">
      <v-list-item v-for="language in appStore.languages" :key="language.id" color="primary" slim
        :active="appStore.language === language.id" @click="appStore.setLanguage(language.id)">
        <template #prepend>
